...
def authorized_actor():
    ...
    # Mocking get_actor for the guard
    # Since guard.get_actor is used in Depends, we can override it in the app
    def mock_get_actor():
        return actor

    app.dependency_overrides[guard.get_actor] = mock_get_actor

    # Set sovereign context for shadow ledger commits
    token = _sovereign_context.set({"token": "TEST-TOKEN", "actor": actor})
...
def test_rc_handoff_fails_if_not_approved(authorized_actor):
    ...
    response = client.post("/api/redcoral/handoff-to-buildx", json=payload)
    assert response.status_code == 400
    assert "Design must be approved before handoff" in response.json()["detail"]

def test_bx_settlement_fails_without_evidence(authorized_actor):
    # 2. BX cannot request FCE settlement unless milestone has site evidence and SHADOW proof.
    response = client.post("/api/buildx/request-fce-settlement?milestone_id=M1&amount_mvr=1000")
    assert response.status_code == 400
...
def test_ax_validation_fails_closed_if_identity_missing():
    ...
    assert response.status_code == 403 # ExecutionGuardMiddleware or get_actor_ctx check

def test_orca_failure_blocks_fce_settlement(authorized_actor):
    ...
    orca_core.validate("SOME_CHECK", actor_id, {"leak_detected": True}) # Should fail for ZERO_LEAK if I used that,
    # but my engine.py says if validation_type == "ZERO_LEAK" and data.get("leak_detected"): passed = False
    orca_core.validate("ZERO_LEAK", actor_id, {"leak_detected": True})
...
def test_shadow_event_integrity(authorized_actor):
...
def test_utility_disconnect_fails_if_leak(authorized_actor):
    ...
    response = client.post("/api/atollx/utilities/drybreak-disconnect", json=payload)
    assert response.status_code == 400
    assert "Zero-leak validation failed" in response.json()["detail"]

def test_pool_system_fails_if_not_reef_safe(authorized_actor):
    ...
    response = client.post("/api/atollx/pool/water-quality", json=payload)
    assert response.status_code == 400
    assert "Reef-safe validation failed" in response.json()["detail"]

def test_water_batch_quarantine_if_contaminated(authorized_actor):
    ...
    response = client.post("/api/atollx/water/quality", json=payload)
    assert response.status_code == 200
    assert response.json()["status"] == "BATCH_QUARANTINED"

def test_dredge_volume_claim_fails_without_survey(authorized_actor):
    ...
    response = client.post("/api/atollx/dredge/volume-claim", json=payload)
    assert response.status_code == 400
    assert "Volume claim requires sonar/bathymetry proof" in response.json()["detail"]

def test_full_handover_requires_qaqc(authorized_actor):
    ...
    response = client.post("/api/buildx/handover", json=payload)
    assert response.status_code == 400
