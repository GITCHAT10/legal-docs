...
async def headers(client):
    ...
    # Note: create_identity_router is included with prefix "/imoxon"
    # and internal router has prefix "/aegis/identity"
    # Resulting path: /imoxon/aegis/identity/create
...
async def test_unsigned_request_rejected(client):
    ...
    assert "requires identity and device" in res.json()["detail"]
...
async def test_maldives_billing_math(client, headers):
    # Base: 1000
    # Formula: (Base * 1.15 * 1.10) = 1265
    # SC (10% on 1265) = 126.5
    # Subtotal = 1391.5
    # TGST (17% on 1391.5) = 236.555 -> 236.56
    # Total = 1628.06
    ...
    assert pricing["landed_cost"] == 1628.06
...
async def test_shadow_audit_creation(client, headers):
    ...
    # Using existing endpoint /imoxon/suppliers/connect
    ...
    assert "imoxon.supplier.connect" in last_block["event_type"]
...
async def test_failed_transaction_rollback(client, headers):
    ...
    from main import shadow_core
    last_block = shadow_core.chain[-1]
    assert "failed" in last_block["event_type"]
