...
async def headers(client):
    ...
    res = await client.post("/aegis/identity/device/bind", params={"identity_id": actor_id}, json={"fingerprint": "test-dev"})
...
async def test_unsigned_request_rejected(client):
    ...
    assert "AEGIS_REQUIRED: Missing Identity, Device or Signature" in res.json()["detail"]
...
async def test_shadow_audit_creation(client, headers):
    ...
    # Plus one internal identity.created for the supplier connection
    # Plus one direct auth success for the action
...
async def test_failed_transaction_rollback(client, headers):
    ...
    # ExecutionGuard wraps the KeyError in a RuntimeError which results in a 500 or it might be handled.
