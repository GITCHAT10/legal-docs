...
def create_restaurant_router(restaurant_engine, get_actor_ctx):
    ...
    async def sync_offline(merchant_id: str, request: OfflineSyncRequest, actor: dict = Depends(get_actor_ctx)):
        from mnos.shared.execution_guard import authorized_context
        with authorized_context(actor):
            return restaurant_engine.bpe.sync_offline_batch(merchant_id, request.transactions)
