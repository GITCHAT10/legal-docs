...
def void_charge(db: Session, line_id: int, reason: str, trace_id: str, tenant_id: str = "default", actor: str = "SYSTEM") -> models.FolioLine:
    ...
    Linked via reversal_of_entry_id.
    """
    try:
        ...
        # 1. Create Counter-Entry (Linked Counter-Signal)
        reversal_line = models.FolioLine(
            ...
            amount=-line.amount,
            description=f"REVERSAL: {reason} (Original Ref: {line.id})",
            is_reversed=True,
            reversal_of_entry_id=line.id # Explicit linking
        )
