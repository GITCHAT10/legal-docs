...
class Folio(Base):
    ...
    guest_id = Column(Integer, ForeignKey("guest.id"), index=True) # Enabled multi-property guest folio
...
class FolioLine(Base):
    ...
    currency = Column(String, default="USD") # Line-level currency
    exchange_rate = Column(Float, default=1.0)
    description = Column(String)
    is_reversed = Column(Boolean, default=False)
    reversal_of_entry_id = Column(Integer, ForeignKey("folioline.id"))
...
class ExchangeRateLock(Base):
    ...
    target_currency = Column(String, nullable=False) # e.g., MVR
    rate = Column(Float, nullable=False) # 1 USD = 15.42 MVR
