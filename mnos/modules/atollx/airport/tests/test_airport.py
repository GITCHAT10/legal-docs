...
def auth_headers():
    return {
        "X-AEGIS-IDENTITY": "AIRPORT-ADMIN-001",
        "X-AEGIS-DEVICE": "AIR-DEV-001",
        "X-TRACE-ID": "AIR-TRACE-001"
    }
...
def test_runway_package_blocked_without_pavement_validation(authorized_actor, auth_headers):
    ...
    response = client.post("/api/atollx/airport/pavement/validate", json=payload, headers=auth_headers)
    assert response.status_code == 400
    assert "Specialist validation required" in response.json()["detail"]

def test_ols_obstacle_hold(authorized_actor, auth_headers):
    ...
    response = client.post("/api/atollx/airport/ols/review", json=payload, headers=auth_headers)
    assert response.status_code == 200
    assert response.json()["status"] == "OLS_OBSTACLE_HOLD"

def test_airnav_interference_hold(authorized_actor, auth_headers):
    ...
    response = client.post("/api/atollx/airport/navigation/review", json=payload, headers=auth_headers)
    assert response.status_code == 200
    assert response.json()["status"] == "AIR_NAVIGATION_HOLD"

def test_orca_blocks_airport_fce(authorized_actor, auth_headers):
    ...
    orca_core.validate("ZERO_LEAK", authorized_actor["identity_id"], {"leak_detected": True, "project_id": "A1"})

    response = client.post("/api/atollx/airport/fce/settlement-request?milestone_id=M1&project_id=A1&amount_mvr=500000", headers=auth_headers)
    assert response.status_code == 400
    assert "ORCA validation failure" in response.json()["detail"]

def test_mcaa_submission_package_requirements(authorized_actor, auth_headers):
    ...
    response = client.post("/api/atollx/airport/mcaa/submission-package", json=payload, headers=auth_headers)
    assert response.status_code == 400
    assert "MCAA submission package incomplete" in response.json()["detail"]

def test_airport_shadow_integrity(authorized_actor, auth_headers):
    # 9. every accepted action must write SHADOW event with trace_id, parent_hash, payload_hash, timestamp_utc
    client.post("/api/atollx/airport/project", json={"project_id": "A1", "name": "Airport 1"}, headers=auth_headers)
