...
class AtollCommanderScoringEngine:
    ...
    def update_hustle_score(self, gm_id: str, island: str):
        ...
        nexus = getattr(self.core, "nexus", None)
        if not nexus:
            # Fallback to island_gm path
            gm = getattr(self.core, "island_gm", None)
            nexus = getattr(gm, "nexus", None) if gm else None

        if not nexus:
            return

        verified_vendors = [v for v in nexus.vendors.values()
