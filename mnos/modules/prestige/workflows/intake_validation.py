...
class IntakeValidation:
    def validate_completion(self, intake: UHNWBookingTemplate) -> Tuple[bool, str]:
        ...
        """
        if not intake.privacy_level:
            return False, "Missing privacy_level"
        if not intake.travel_dates:
            return False, "Missing travel_dates"
        if not intake.adults:
            return False, "Missing adults count"
        if not intake.arrival_mode:
            return False, "Missing arrival_mode"
        if not intake.villa_preference:
            return False, "Missing villa_preference"
        if not intake.transfer_preference:
            return False, "Missing transfer_preference"
        if not intake.budget_band:
            return False, "Missing budget_band"
        if not intake.payment_preference:
            return False, "Missing payment_preference"
