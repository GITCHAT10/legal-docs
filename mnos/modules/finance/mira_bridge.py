...
class MiraBridgeEngine:
    ...
    def record_transaction(self, order_data: dict):
        ...
        ledger_ref = self.daily_ledgers[key]
        ledger_ref["total_base"] += pricing["base"]
        ledger_ref["service_charge"] += pricing["service_charge"]
        ledger_ref["tgst"] += pricing["tax_amount"]
        ledger_ref["green_tax"] += pricing.get("green_tax", 0.0)
        ledger_ref["total_sales"] += pricing["total"]
        ledger_ref["tx_count"] += 1
