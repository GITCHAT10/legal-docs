...
def create_ticket(db: Session, room_id: int, title: str, description: str, priority: enums.TicketPriority, severity: enums.TicketSeverity = enums.TicketSeverity.MEDIUM, trace_id: Optional[str] = None, actor: str = "SYSTEM") -> models.MaintenanceTicket:
    try:
        ...
        deadline = datetime.now(UTC) + timedelta(hours=sla_hours.get(priority.value, 24))
...
def update_ticket_status(db: Session, ticket_id: int, new_status: enums.TicketStatus, actor: str = "SYSTEM") -> models.MaintenanceTicket:
    try:
        ...
        trace_id = f"MAINT-STATUS-{uuid4().hex[:8]}"
        ...
        shadow_service.commit_evidence(db, trace_id, {
            ...
            "before_state": before_state, "after_state": {"status": new_status}
        })
        db.commit()
        db.refresh(ticket)
        return ticket
