...
use Modules\Academy\App\Models\ShadowSyncQueue;
use Modules\Academy\App\Services\MNOS\MnosTransaction;
use Illuminate\Http\Request;


class ShadowSyncQueueController extends BaseAcademyController
{
    public function index()
    {
        return $this->mnosResponse(ShadowSyncQueue::all());
    }
    ...
    public function store(Request $request)
    {
        $entity = MnosTransaction::execute(function() use ($request) {
            ...
            return ShadowSyncQueue::create($data);
        });
    ...
    {
        return $this->mnosResponse(ShadowSyncQueue::findOrFail($id));
    }

    public function update(Request $request, $id)
    {
        $entity = MnosTransaction::execute(function() use ($request, $id) {
            $item = ShadowSyncQueue::findOrFail($id);
    ...
    public function destroy($id)
    {
        MnosTransaction::execute(function() use ($id) {
            return ShadowSyncQueue::findOrFail($id)->delete();
        });
