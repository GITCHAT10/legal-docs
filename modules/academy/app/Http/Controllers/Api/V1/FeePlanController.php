...
use Modules\Academy\App\Models\FeePlan;
use Modules\Academy\App\Services\MNOS\MnosTransaction;
use Illuminate\Http\Request;


class FeePlanController extends BaseAcademyController
{
    public function index()
    {
        return $this->mnosResponse(FeePlan::all());
    }
    ...
    {
        return $this->mnosResponse(FeePlan::findOrFail($id));
    }

    public function update(Request $request, $id)
    {
        $entity = MnosTransaction::execute(function() use ($request, $id) {
            $item = FeePlan::findOrFail($id);
    ...
    public function destroy($id)
    {
        MnosTransaction::execute(function() use ($id) {
            return FeePlan::findOrFail($id)->delete();
        });
